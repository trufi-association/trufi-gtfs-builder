...
function geojsonToGtfs(features, inputStops, gtfsConfig, gtfsBuilders) {
    ...
    let stops = stopsBuilder(featuresArray, inputStops, gtfsConfig.stopNameBuilder, gtfsConfig.stopsConfig ?? gtfsConfig.customStops);
    ...
    if (gtfsConfig.mergeNearbyStops && gtfsConfig.mergeNearbyStops > 0) {
        const merged = (0, gtfsBuilders_1.mergeNearbyStops)(stops, stopTimes, trips, gtfsConfig.mergeNearbyStops);
        stops = merged.stops;
    ...
    const originalTripStops = new Map();
    ...
    const gapFilledStopTimes = [];
    // Group merged stop_times by trip
    const mergedByTrip = new Map();
    for (const st of mergedStopTimes) {
        if (!mergedByTrip.has(st.trip_id))
            mergedByTrip.set(st.trip_id, []);
    ...
    for (const [tripId, keptSts] of mergedByTrip) {
        ...
        for (let k = 0; k < keptSts.length; k++) {
            ...
            if (k < keptSts.length - 1) {
                ...
                if (!stopA || !stopB)
                    continue;
                const dist = (0, spatialMatcher_1.distanceBetweenCoords)(stopA.stop_lat, stopA.stop_lon, stopB.stop_lat, stopB.stop_lon);
                ...
                if (origIdxA < 0 || origIdxB < 0 || origIdxB <= origIdxA + 1)
                    continue;
                ...
                if (needed >= intermediates.length) {
                    ...
                    toRestore = Array.from({ length: needed }, (_, i) => intermediates[Math.round(step * (i + 1)) - 1]);
                }
