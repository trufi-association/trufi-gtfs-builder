...
exports.default = routeExtractor;
...
function routeExtractor(route_elements, ways, stops) {
    const routeWays = [];
    const routeStops = [];
    for (const element of route_elements.members) {
        if (element.type === 'way' && !element.role) {
            ...
            if (current_way == null) {
                throw {
                    extractor_error: extractor_error_1.default.way_not_exist,
                    uri: `https://overpass-turbo.eu/?Q=${encodeURI(`//${extractor_error_1.default.way_not_exist}\nrel(${route_elements.id});out geom;way(${element.ref});out geom;`)}&R`,
                };
            }
            routeWays.push({
                type: 'way',
    ...
    if (routeWays.length === 0) {
        throw {
            extractor_error: extractor_error_1.default.route_with_empty_ways,
            uri: `https://overpass-turbo.eu/?Q=${encodeURI(`//${extractor_error_1.default.route_with_empty_ways}\nrel(${route_elements.id});out geom;`)}&R`,
        };
    }
    for (let index = 1; index < routeWays.length; index++) {
        ...
        if (!checkCurrentWay) {
            throw {
                extractor_error: extractor_error_1.default.not_next,
                uri: `https://overpass-turbo.eu/?Q=${encodeURI(`//${extractor_error_1.default.not_next}\nrel(${route_elements.id});out geom;\nway(${lastWay.id});out geom;\nway(${currentWay.id});out geom;`)}&R`,
            };
        }
    }
    let tmp_nodes = [];
...
//# sourceMappingURL=route_extractor.js.map